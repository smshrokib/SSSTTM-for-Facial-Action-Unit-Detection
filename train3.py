    ...
    for epoch in range(start_epoch, epochs):
        ...
        while data is not None:
            ...
            if model.task.lower() == 'all':
                bar.set_postfix(total=total_loss.avg, ex=ex_loss_record.avg, au=au_loss_record.avg,
                                va=va_loss_record.avg)
            else:
            ...
            data = prefetcher.next()
        logging.info(
            f'Train Total Loss,{total_loss.avg}, Ex:{ex_loss_record.avg}, AU:{au_loss_record.avg}, VA:{va_loss_record.avg}')
        ...
        scores, val_loader_iter, val_loss = evaluate(model, val_loader, val_loader_iter, device,
                                                     num_step=int(
                                                         sum(dataset.val_ids * downsample) / (args['batch_size'] * 4)))
...
def main(args):
    ...
    if opt['model_name'] == 'avformer':
        ...
        model = VGGVisualFormer(modality=args['modality'], task=task, video_pretrained=False,
        ...
        model = SpatialTemporalFormer(modality=args['modality'], task=task, video_pretrained=False,
                                      num_patches=args['n_frames'])
