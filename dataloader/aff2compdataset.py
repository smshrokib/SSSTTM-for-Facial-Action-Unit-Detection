...
class Aff2CompDataset(Dataset):
    def __init__(self, opt):
        ...
        if not os.path.isfile(self.cached_metadata_path):
            ...
            self.image_path = [i.replace('\\', '/') for i in split_dict['image_path']]
    ...
    def __getitem__(self, index):
        ...
        data['aug'] = self.clip_transform(RandomColorAugmentation(clip))
        # data['video_id'] = video_id
        # data['current_frame_path'] = current_frame_path
