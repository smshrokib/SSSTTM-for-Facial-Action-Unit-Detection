...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, num_modules, num_branches, block, 
            num_blocks, num_channels, fuse_method, emb_dim=512, **kwargs):
        ...
        self.classifier = nn.Linear(2048, self.emb_dim)
    ...
    def forward(self, x):
        ...
        x = self.bn1(x)
        x = self.relu(x)
        x = self.conv2(x) #(64, 28, 28)
        x = self.bn2(x)
        x = self.relu(x)
        x = self.layer1(x) #(256, 28, 28)

        x_list = []
        for i in range(self.num_branches[1]):
            if self.transition1[i] is not None:
                x_list.append(self.transition1[i](x)) #(18, 28, 28)  #(36, 14, 14)
            else:
                x_list.append(x)
        y_list = self.stage2(x_list) #(18, 28, 28)  #(36, 14, 14)

        x_list = []
        for i in range(self.num_branches[2]):
            if self.transition2[i] is not None:
                x_list.append(self.transition2[i](y_list[-1])) #(72, 7, 7)
            else:
                x_list.append(y_list[i]) #(18, 28, 28)  #(36, 14, 14)
        y_list = self.stage3(x_list) #(18, 28, 28)  #(36, 14, 14) #(72, 7, 7)
        ...
        # Classification Head
        y = self.incre_modules[0](y_list[0])  #(128, 28, 28)
        for i in range(len(self.downsamp_modules)):
            y = self.incre_modules[i+1](y_list[i+1]) + \
                        self.downsamp_modules[i](y) #(256, 14, 14)  #(512, 7, 7)
...
def hrnet18(pretrain_path=None, **kwargs):
    ...
    if pretrain_path:
        ...
        pretrain_dict = {k.replace('backbone.', ''): v for k, v in pretrain.items() if k.replace('backbone.', '') in net_dict.keys()}
