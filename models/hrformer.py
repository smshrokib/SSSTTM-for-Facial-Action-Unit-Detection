...
class HrFormer(nn.Module):  # S-Former after stage3

    def __init__(self, block, layers, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None, norm_layer=None,
                 num_patches=7 * 7, dim=512, depth=1, heads=8, mlp_dim=512, dim_head=32, dropout=0.0):
    ...
    def forward(self, x):
        ...
        x = x.reshape((b_l, c, h * w)) #(512, 49)
        # torch.Size([32, 512, 49])
        x = x.permute(0, 2, 1)
        b, n, _ = x.shape
        # x shape: torch.Size([32, 49, 512])
        # pos_embedding shape torch.Size([1, 49, 512])
        ...
        # torch.Size([32, 512, 7, 7])
        x = self.hr_emb.final_layer(x) #(1024, 4, 4)
...
class TFormer(nn.Module):
    ...
    def forward(self, x):
        # x = x.contiguous().view(-1, self.num_patches, self.dim)

        # Infer patch count from the incoming tensor to avoid shape crashes
        # when the pipeline produces fewer/more patch tokens than self.num_patches.
        if x.dim() == 2:
            b = x.shape[0]
            total = x.shape[1]
            if total % self.dim != 0:
                raise RuntimeError(f"t_former: cannot reshape tensor of shape {tuple(x.shape)} with dim={self.dim}")
            inferred_patches = total // self.dim
            x = x.contiguous().view(b, inferred_patches, self.dim)
        else:
            # keep original behavior if tensor already has patch dimension
            x = x.contiguous()
...
class VideoModel(nn.Module):
    def __init__(self, num_patches=16):
        ...
        self.s_former = HrFormer(BasicBlock, [2, 2, 2, 2])
        self.au_head = AU_former(input_dim=1024, dropout=0.2)
...
class SpatialTemporalHrFormer(nn.Module):
    def __init__(self, modality='A;V;M', video_pretrained=True, task='EX', num_patches=16):
        super(SpatialTemporalHrFormer, self).__init__()
    ...
    def get_mt_mt_loss(self, y_pred, y_true, ema_pred):  # mean-teacher multi-task loss
        device = y_pred.device
        ...
        if len(invalid_ex_indices) > 0:
            ...
            y_pred_ex = torch.index_select(y_pred[:, 12:19], 0, invalid_ex_indices)
            ema_logits_ex = torch.index_select(ema_pred[:, 12:19], 0, invalid_ex_indices)
        ...
        if len(invalid_va_indices) > 0:
            ...
            y_pred_va = torch.index_select(y_pred[:, 19:21], 0, invalid_va_indices)
            ema_pred_va = torch.index_select(ema_pred[:, 19:21], 0, invalid_va_indices)
