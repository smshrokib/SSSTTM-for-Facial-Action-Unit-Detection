...
class MultiLabelF1_test(object):
    def __init__(self, ignore_index=None, average='binary'):
        self.ignore_index = ignore_index
        self.average = average
        self.y_pred = []
    ...
    def get(self):
        ...
        f1 = []
        for i in range(y_pred.shape[1]):
            _, f1_i = acc_f1_score(y_true=y_true[:, i], y_pred=y_pred[:, i],
                                       average=self.average,
                                       normalize=False,
                                       ignore_index=self.ignore_index)
            f1.append(f1_i)

        _, micro_f1 = acc_f1_score(y_true=y_true.flatten(), y_pred=y_pred.flatten(),
                                   average=self.average,
                                   normalize=False,
                                   ignore_index=self.ignore_index)
        macro_f1 = np.mean(f1)
        return f1, micro_f1, macro_f1
