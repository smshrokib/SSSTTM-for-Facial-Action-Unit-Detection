...
def evaluate(model, loader, loader_iter, device, num_step=1000):
    ...
    for step in bar:
        ...
        # Use argmax to get predicted classes (avoid torch.max attribute issue)
        pred = logits_ex.argmax(dim=1).detach().cpu().numpy().reshape(-1)
        label = label_ex.detach().cpu().numpy().reshape(-1)

        metric_ex.update(pred, label)
        metric_va.update(y_pred=logits_va.tanh().detach().cpu().numpy(), y_true=labels['VA'].detach().cpu().numpy())
        metric_au.update(y_pred=np.round(logits_au.sigmoid().detach().cpu().numpy()), y_true=labels['AU'].detach().cpu().numpy())
...
def train(args, model, dataset, optimizer, epochs, device):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        while data is not None:
            ...
            t1 = time.time()
            data = prefetcher.next()
        logging.info(f'Train Total Loss,{total_loss.avg}, Ex:{ex_loss_record.avg}, AU:{au_loss_record.avg}, VA:{va_loss_record.avg}')
    ...
    plt.ylabel('loss')
    plt.legend(loc='best')
    plt.savefig('HRFormer_F16_loss.png')
    plt.close()
...
def main(args):
    ...
    print('clip size',args['n_frames'],args['image_size'])
    log_file_name = args['model_name']+'_'+args['modality']+'_log.txt'
    logging.basicConfig(filename=os.path.join(args['exp_dir'], log_file_name), level=logging.INFO,
                    format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
    logging.getLogger()
    
    # model
    if args['model_name'] == 'avformer':
        model = TwoStreamAuralVisualFormer(modality=args['modality'], task=task, video_pretrained=False, audio_pretrained=False, num_patches=args['n_frames'])
    elif args['model_name'] == 'vformer':
        model = VisualFormer(modality=args['modality'], task=task)
    elif args['model_name'] == 'vggformer':
        model = VGGVisualFormer(modality=args['modality'], task=task, video_pretrained=False, num_patches=args['n_frames'])
    elif args['model_name'] == 'emonet':
        model = ImageEmoNetModel(modality=args['modality'], task=task)
    elif args['model_name'] == 'tformer':
        model = SpatialTemporalFormer(modality=args['modality'], task=task, video_pretrained=False, num_patches=args['n_frames'])
    elif args['model_name'] == 'hrformer':
        model = SpatialTemporalHrFormer(modality=args['modality'], task=task, video_pretrained=False,
                                      num_patches=args['n_frames'])
    elif args['model_name'] == 'sformer':
